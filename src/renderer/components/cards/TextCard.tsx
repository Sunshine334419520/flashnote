      ...
      <div className="rounded-2xl border border-border bg-card p-4 space-y-3 card-hover">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {editing ? (
            ...
            <>
              ...
              <span className="text-[9px] px-1.5 py-0.5 rounded font-medium bg-type-text/10 text-type-text shrink-0 select-none">{t('type.text')}</span>
            </>
          )}
        </div>
        ...
        {confirming ? (
          ...
          <div className="space-y-2">
            <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full text-xs bg-muted/50 rounded-lg px-2 py-1.5 outline-none border border-border focus:border-ring/30 resize-none min-h-[80px] leading-relaxed" onKeyDown={(e) => { if (e.key === 'Escape') handleCancel() }} />
          ...
          <div>
            <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-line">{preview}</p>
            {isLong && (
              <button onClick={() => setShowModal(true)} className="mt-1.5 flex items-center gap-1 text-[10px] text-type-text hover:text-type-text/80 transition-colors">
                <Maximize2 size={11} /> {t('card.expand')}
              </button>
            )}
            {tagList.length > 0 && (
              <div className="flex items-center gap-1 flex-wrap mt-2">
                {tagList.map((tg) => <span key={tg} className="text-[10px] px-1.5 py-0.5 rounded bg-muted/50 text-muted-foreground">{tg}</span>)}
              </div>
            )}
          </div>
        )}
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={handleCopy} className={cn('flex items-center gap-1 text-[10px] px-1.5 py-1 rounded-md transition-colors', copied ? 'text-type-text bg-type-text/10' : 'text-muted-foreground hover:text-foreground hover:bg-muted')}>{copied ? <Check size={11} /> : <Copy size={11} />}{copied ? t('card.copied') : t('card.copy')}</button>
