  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 space-y-3 card-hover">
      <div className="flex items-center gap-2 min-w-0">
        <Terminal size={16} className="text-type-command shrink-0" />
        {editing ? (
          <input
            ref={titleInputRef}
            value={editTitle}
          ...
          <>
            ...
            <span className="text-micro px-1.5 py-0.5 rounded font-medium bg-type-command/10 text-type-command shrink-0 select-none">{t('type.command')}</span>
          </>
        )}
      </div>
      ...
      {confirming ? (
        <div className="text-center py-2">
          <p className="text-label text-muted-foreground mb-2">{t('confirm.deleteCmd')}</p>
        ...
        <div className="space-y-2">
          <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full text-label font-mono bg-muted/50 rounded-lg px-2 py-1.5 outline-none border border-border focus:border-ring/30 resize-none min-h-[60px]" onKeyDown={(e) => { if (e.key === 'Escape') handleCancel() }} />
        ...
        <pre className="text-label font-mono text-muted-foreground bg-muted/30 rounded-lg px-3 py-2 overflow-x-auto whitespace-pre-wrap max-h-24 overflow-y-auto select-all leading-relaxed">{note.content}</pre>
      )}
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-micro text-muted-foreground min-w-0">
              <span className="truncate">{note.category}</span><span>·</span><span className="shrink-0">{timeAgo}</span>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={handleCopy} className={cn('flex items-center gap-1 text-micro px-1.5 py-1 rounded-md transition-colors', copied ? 'text-type-command bg-type-command/10' : 'text-muted-foreground hover:text-foreground hover:bg-muted')}>
                {copied ? <Check size={12} /> : <Copy size={12} />}{copied ? t('card.copied') : t('card.copy')}
              </button>
