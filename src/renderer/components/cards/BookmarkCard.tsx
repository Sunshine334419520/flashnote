...
const TYPE_LABEL = 'Bookmark'

export function BookmarkCard({ note, onUpdate, onDelete }: Props): ReactElement {
  ...
  const url = typed?.url ?? note.content
    ...
    <div className="rounded-2xl border border-border bg-card p-4 space-y-3 card-hover">
      <div className="flex items-center gap-2 min-w-0">
        ...
        {editing ? (
          ...
          <>
            ...
            <span className="text-[9px] px-1.5 py-0.5 rounded font-medium bg-type-bookmark/10 text-type-bookmark shrink-0 select-none">
              {TYPE_LABEL}
            </span>
          </>
        )}
      ...
      {confirming ? (
        <div className="text-center py-2">
          <p className="text-xs text-muted-foreground mb-2">确认删除这个书签？</p>
        ...
        <div className="space-y-2">
          <input
            ...
            className="w-full text-[12px] bg-muted/50 rounded-lg px-2 py-1.5 outline-none border border-border focus:border-ring/30"
            placeholder="URL"
        ...
        <div>
          {domain && (
            <p className="text-[11px] font-medium text-type-bookmark mb-1 truncate">{domain}</p>
          )}
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button
                onClick={handleOpen}
                className="flex items-center gap-1 text-[10px] px-1.5 py-1 rounded-md text-muted-foreground hover:text-type-bookmark hover:bg-type-bookmark/10 transition-colors"
              >
              ...
              <button
                ...
                className={cn(
                  ...
                  copied ? 'text-type-bookmark bg-type-bookmark/10' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                )}
              >
