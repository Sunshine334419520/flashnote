...
export function BookmarkCard({ note, onUpdate, onDelete }: Props): ReactElement {
  ...
  const handleOpen = () => { void window.electronAPI.shell.openExternal(url) }
  const handleCopyLink = async () => { await navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 1500) }
