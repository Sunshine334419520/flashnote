  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 space-y-3 card-hover">
      <div className="flex items-center gap-2 min-w-0">
        <Lock size={16} className="text-type-credential shrink-0" />
        {editing ? (
          ...
          <>
            ...
            <span className="text-micro px-1.5 py-0.5 rounded font-medium bg-type-credential/10 text-type-credential shrink-0 select-none">{t('type.credential')}</span>
          </>
        )}
      </div>
      ...
      {confirming ? (
        <div className="text-center py-2">
          <p className="text-label text-muted-foreground mb-2">{t('confirm.deleteCred')}</p>
        ...
        <div className="space-y-2">
          <input value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full text-label font-mono bg-muted/50 rounded-lg px-2 py-1.5 outline-none border border-border focus:border-ring/30" onKeyDown={(e) => { if (e.key === 'Escape') handleCancel() }} />
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={handleCopy} className={cn('flex items-center gap-1 text-micro px-1.5 py-1 rounded-md transition-colors', copied ? 'text-type-credential bg-type-credential/10' : 'text-muted-foreground hover:text-foreground hover:bg-muted')}>{copied ? <Check size={12} /> : <Copy size={12} />}{copied ? t('card.copied') : t('card.copy')}</button>
